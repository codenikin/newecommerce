...
interface ProductCardRowStyleTwoProps {
  className?: string
  datas: any
}

export default function ProductCardRowStyleTwo({ className, datas }: ProductCardRowStyleTwoProps) {
  const image = datas.gallery?.[0]?.image
  const imageUrl = typeof image === 'object' && image !== null ? image.url : null
  return (
    <div data-aos="fade-up" className={`product-card-row-two w-full  ${className || ''}`}>
      <div className="w-full h-[105px] bg-white border border-primarygray px-5 ">
        <div className="w-full h-full flex space-x-5 justify-center items-center">
          <div className="w-[75px] h-[75px]">
            {imageUrl ? (
              <img src={imageUrl} alt={datas.title} className="w-full h-full object-cover" />
            ) : null}
          </div>
